...
@register
class KpassetScraper(BaseScraper):
    name = "우정사업본부"
    short_name = "KPASSET"
    base_url = "https://www.kpasset.go.kr/board.es?mid=a10510000000&bid=0041&intro_yn=Y"

    async def wait_for_content(self, page: Page) -> None:
        try:
            # Government sites often use specific board frameworks
            await page.wait_for_selector("table, .board_list, .bbsListTbl", timeout=15_000)
        except Exception:
