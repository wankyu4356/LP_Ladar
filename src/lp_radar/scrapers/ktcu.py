...
@register
class KtcuScraper(BaseScraper):
    name = "교직원공제회"
    short_name = "KTCU"
    base_url = "https://www.ktcu.or.kr/PPW-CSB-000101"

    async def wait_for_content(self, page: Page) -> None:
        try:
            await page.wait_for_selector("table, .board_list, .notice_list, .bbs-list", timeout=15_000)
        except Exception:
    ...
    async def extract_announcements(self, page: Page) -> list[Announcement]:
        ...
        if not rows:
            rows = await page.query_selector_all(".board_list li, .notice_list li")
